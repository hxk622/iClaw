...
- `window`
  - 计划上线时间窗
- `test_report`
  - 对应测试报告路径，固定指向 `docs/version_record/test_report/<版本号>.md`
...
- `docs/version_record/test_report/<发布版本号>.md`
...
- `test_report: docs/version_record/test_report/<发布版本号>.md`

测试证据存储规则：

- 测试报告 `md` 纳入 git
- 截图等视觉证据默认不纳入 git
- 截图统一上传到 dev MinIO，并在测试报告中回填对象路径

发版硬门槛补充：

- `P0` 优先级的正式用户级自动化测试必须全部跑通，才允许进入正式发版
- 如某条 `P0` 用例因环境问题被豁免，必须在测试报告与发布单中同时写明原因、影响范围、替代验证和审批人
