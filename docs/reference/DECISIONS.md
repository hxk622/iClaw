...
- 详细的 wrapper 架构、`3.8` 经验复盘、auth bridge / theme bridge / diagnostics 分层，见：
  - [openclaw-wrapper-architecture.md](../architecture/openclaw-wrapper-architecture.md)
...
- [oem-capability-architecture.md](../architecture/oem-capability-architecture.md)
...
- `cloud skill` 的 artifact 只能来自云端对象存储 / 外部 URL / 外部构建上传，不允许再依赖仓库内 `skills/` 目录重新打包
...
针对 `龙虾商店 / 智能投资专家` 这类 agent/expert 商店，进一步冻结以下原则：

- 用户点击“安装 agent”后，安装结果必须是 **OpenClaw native agent**
- native agent 必须在本地 `~/.openclaw/agents/<agentId>/...` 体系中落地，而不是只在云端保留一条安装记录
- `agent 商店` 与 `skill 商店` 必须有真实分层：
  - `skill 商店` 安装的是 skill
  - `agent 商店` 安装的是 native agent
- agent 可以依赖多个 skill，但 agent 不能退化成 “skill + system_prompt” 的伪装形态
- 同一用户可见对话窗口允许 agent handoff：
  - conversation 连续
  - 当前 thread 上下文共享
  - agent 长期记忆、auth profile、workspace、session store 仍按 native agent 隔离
- 切换 agent 不应强制打开空白新聊天窗口；用户仍在同一 conversation 中，但 active native agent 改变
- 若后续需要 handoff summary，应作为系统内部上下文注入，不应要求用户重复喂养完整上下文
...
- Apple 签名、公证、updater key 只允许放在 `.env.signing.*`，禁止继续塞进 `.env.xxx`
- 打包链禁止跨环境偷读配置，例如 `prod` 打包禁止回读 `.env.dev`
- `pnpm dev:api` 与桌面包 sidecar 必须使用同一套 launcher 语义，统一执行 `openclaw.mjs gateway ...`
- 禁止 dev / prod 分别维护不同的 OpenClaw 启动链，避免理解漂移和验收偏差
- `dev / test / prod` 必须共用同一个 `openclaw.json` 生成器；launcher 只负责收集输入并调用生成器
- OEM snapshot、runtime secret、workspace、sidecar 端口等允许作为输入不同，但不允许再分别在 Bash / Rust / 前端里复制一份配置组装逻辑

## 14. Desktop Surface Cache 架构冻结

针对 `apps/desktop` 的页面切换与多 surface 生命周期，冻结以下原则：

- 左侧导航切换不再默认卸载 chat surface；默认语义改为“surface 切换”，不是“销毁重建”
- `chat`、`menu`、`overlay` 必须分池管理，不能共享一个全局 cache / LRU
- 默认池上限冻结为：
  - `chatPool.max = 50`
  - `menuPool.max = 8`
  - `overlayPool.max = 5`
- 淘汰策略必须使用 `guarded LRU`，不能使用裸 LRU
- 以下实例禁止被淘汰：
  - 当前 `active` surface
  - `busy=true` 的 surface
  - `hasPendingBilling=true` 的 surface
- 隐藏 surface 默认进入 `warm-hidden`，继续保留最小必要运行态，不允许直接卸载
- 命中上限后，必须先持久化 snapshot，再执行淘汰
- 本架构只允许在桌面壳层实现；不通过修改 OpenClaw kernel 来解决 surface 生命周期问题

详细设计见：

- [desktop-surface-cache-architecture.md](../architecture/desktop-surface-cache-architecture.md)

## 15. Chat Domain Concept Freeze

针对聊天域术语与主键边界，进一步冻结以下原则：

- 聊天域只承认 3 个一等概念：
  - `conversation`
  - `turn`
  - `session`
- `conversation` 是用户可感知的独立对话线程，也是左侧聊天入口的主语义
- `turn` 是 conversation 内部的一次完整问答轮次，后续历史任务、计费、artifact、状态都应围绕 `turn` 建模
- `session` 是 runtime 技术容器，不是产品主语义，不得直接充当左侧列表身份
- 禁止继续在新代码中扩散 `task / run / thread / window / chatSession` 这类高歧义概念
- 任何需要表达“历史任务”的地方，若展示的是轮次列表，必须使用 `turn`
- 任何需要表达“左侧独立窗口”的地方，必须使用 `conversation`

详细术语规范见：

- [chat-domain-concepts.md](../architecture/chat-domain-concepts.md)

## 16. Chat Current-State Single-Truth Freeze

聊天主界面冻结以下架构原则：

- 当前对话只能有一个产品层单一真相
- 左侧 conversation 选中态、右侧 chat view、路由落点、runtime 恢复落点，都必须从同一个 `conversation` 主状态派生
- 禁止在产品层并行维护第二套“displayed / visible / rendered current conversation”状态
- `surface key`、`snapshot hit`、`runtime ready`、`reveal-ready` 只能作为从属状态，不能与当前 `conversation` 并列
- 允许缓存多个 surface / session，但不允许缓存机制反过来定义当前对话
- 任何 UI 白闪、映射错乱、切换错位问题，优先检查是否出现了双轨 current state

工程口令冻结如下：

- 永远不要双轨 current state
- 当前对话只能有一个单一真相
