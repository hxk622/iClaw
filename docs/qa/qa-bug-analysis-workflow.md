...
- highlight the key symptom or scope with `【】` when helpful
- avoid vague titles such as "has issue", "not working", or "exception"
- prefer the structure `【module / scene】specific symptom`

Good examples:

- `【Startup】First launch enters a blank window after runtime check`
- `【Chat】Restoring the window leaves the output area stuck and unclickable`
- `【Login】OAuth callback page stays blank after successful sign-in`
...
- do not make it too short to lose context
- do not make it too long to bury the core issue
- prefer short paragraphs or labeled sections instead of one long block
