# iClaw Browser Extension

这是 iClaw 的浏览器插件根目录工程。

当前阶段目标：

- 提供一个可构建、可加载的 MV3 插件骨架
- 支持 action click 打开页面侧悬浮面板
- 支持把“页面保存 / 划词摘录”通过 iClaw 知识库导入协议送入客户端 Raw / 素材
- 插件名称、action title、图标随当前 OEM 品牌变化

当前版本刻意保持最小：

- 不依赖 OpenAlpha 代码
- 只预埋桌面授权桥调用面，不包含正式桥服务实现
- 不依赖 React / Vite
- 先完成最小采集壳和接入协议验证

## 目录

- `public/manifest.json`：插件清单
- `public/background.js`：action click 与内容脚本补注入
- `public/content-script.js`：页面侧悬浮采集面板与导入协议发送
- `scripts/build.mjs`：复制 `public/` 到 `dist/`

## 构建

```bash
pnpm --dir extension build
```

插件会读取当前品牌配置并输出品牌化产物，例如：

- `caiclaw` -> 插件名称显示为 `理财客`

## 加载

1. 打开 `chrome://extensions`
2. 开启开发者模式
3. 选择“加载已解压的扩展程序”
4. 选择 `extension/dist`

## 当前支持

- 桌面授权桥 auth 状态骨架（检测 / 未登录 / 待授权 / 已连接）
- 保存当前页面为 Raw / 素材
- 将当前选区作为 snippet 导入知识库
- 通过 `window.postMessage` 向 iClaw 客户端发送导入消息

## 当前限制

- 还没有桌面授权桥的正式本地服务实现
- 当前 auth 只是插件侧状态机与 background 调用面预埋
- 当前采集动作主要用于打通知识库 Raw 接入协议
