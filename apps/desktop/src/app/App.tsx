...
const SIDECAR_BOOT_HEALTHCHECK_ATTEMPTS = 180;
const SIDECAR_BOOT_HEALTHCHECK_INTERVAL_MS = 1_000;
const SIDECAR_BOOT_HEALTHCHECK_TIMEOUT_MS = 120_000;
    ...
    <SettingsProvider>
      <div className="relative h-screen overflow-hidden">
        ...
        <AuthedView
          ...
          transientToasts={transientToasts}
          onDismissTransientToast={dismissTransientToast}
          onShowTransientToast={showTransientToast}
        />
        {shouldShowStartupGate ? (
...
interface AuthedViewProps {
  ...
  transientToasts: AppNotificationRecord[];
  onDismissTransientToast: (id: string) => void;
  onShowTransientToast: (input: { tone: 'success' | 'error' | 'info'; title: string; text: string }) => void;
}
...
function AuthedView({
    ...
    if (view === 'settings') {
      return (
        <SettingsPanel
          ...
          onShowToast={onShowTransientToast}
        />
      );
    }
    return null;
  ...
  return (
    <div className="relative h-screen overflow-hidden bg-[var(--bg-page)]">
      ...
      />
      <GlobalToastViewport notifications={transientToasts} onDismiss={onDismissTransientToast} />
    </div>
  );
}
