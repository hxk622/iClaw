---
name: 美股金融数据工具包
description: 美股市场分析的金融数据工具包。提供脚本获取实时股票数据(yfinance)、SEC文件和内部人交易(EDGAR)、财务报表计算器(DuPont、Z-Score、M-Score、F-Score)、组合分析(VaR、压力测试、健康评分)、多因子筛选和宏观指标(FRED)。当需要实时美股数据来支撑投资分析时使用。所有数据源都是免费的——无需API密钥。
license: Apache-2.0
visibility: showcase
tags: 美股, 数据工具
---
