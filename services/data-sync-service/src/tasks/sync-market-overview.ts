...
function getPool() {
  if (!poolInstance) {
    poolInstance = createPgPool(controlPlaneConfig.databaseUrl, 'data-sync:market-overview');
  }
  return poolInstance;
}
...
export async function syncMarketOverview(): Promise<SyncTaskExecutionResult> {
  ...
  try {
    ...
    try {
      ...
      await client.query(`delete from market_overview_snapshot where snapshot_at < now() - interval '14 days'`);
      await client.query(`delete from market_index_snapshot where snapshot_at < now() - interval '14 days'`);
