...
const before = await evalJSON(
  ...
  `(() => {
    ...
    return {
      connected: document.querySelector('openclaw-app')?.connected ?? null,
      menu: !!document.querySelector('.iclaw-chat-selection-menu'),
...
const selectionResult = await evalJSON(
  cdp,
  `(() => {
    const target = Array.from(document.querySelectorAll('.openclaw-chat-surface .chat-text'))
      ...
      .reverse()
      .find((entry) => entry.text.length >= 4 && entry.rect.height > 0);
    if (!target) {
      return { ok: false, reason: 'no chat text found' };
    }
...
const menuState = await evalJSON(
  ...
  `(() => {
    ...
    return {
      exists: !!menu,
      text: menu?.textContent?.replace(/\\s+/g, ' ').trim() ?? null,
...
const after = await evalJSON(
  ...
  `(() => {
    const editor = document.querySelector('.iclaw-composer__editor');
    const token = editor?.querySelector('.iclaw-inline-token');
    return {
      menuStillOpen: !!document.querySelector('.iclaw-chat-selection-menu'),
      editorText: editor?.textContent ?? null,
      editorHtml: editor?.innerHTML ?? null,
      tokenLabel: token?.querySelector('.iclaw-inline-token__label')?.textContent ?? null,
