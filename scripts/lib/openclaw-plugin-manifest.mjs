...
const defaultManifestPath = path.join(repoRoot, 'plugins', 'manifest.json');
...
function normalizeSource(rawSource, pluginId) {
  ...
  if (type === 'path') {
    ...
    return { type, path: path.resolve(repoRoot, targetPath) };
  }
...
export function resolvePackagedPluginsManifestPath(env = process.env) {
  ...
  return override ? path.resolve(override) : defaultManifestPath;
}
...
export function loadPackagedPlugins({ manifestPath, env = process.env } = {}) {
  ...
  return plugins.map((rawPlugin, index) => {
    ...
    return {
      id,
      source: normalizeSource(plugin.source, id),
