...
const INITIAL_SURFACE_BOOT_TIMEOUT_MS = 1800;
...
function normalizeToolCards(group: HTMLElement): void {
  ...
  if (group.classList.contains('tool') || !hasVisibleMessageContent(group)) {
...
export function OpenClawChatSurface({
  ...
  useEffect(() => {
    ...
    if (!shellAuthenticated || status.lastError || renderState.groupCount > 0) {
      setInitialSurfaceRestorePending(false);
      return;
    }
  ...
  const showBootMask =
    shellAuthenticated && !status.lastError && (!status.connected || initialSurfaceRestorePending);
